...
Lokal runtime: `cd api-python && python -m unittest discover -s runtime_tests -t runtime_tests`. `-t` berilmaganda discovery `Start directory is not importable: 'runtime_tests'` bilan yiqiladi, chunki `runtime_tests` package emas. Runner: `node --test apps/runner/test.js`. Hozirgi offline to‘plam: **2814 test** (`failures=1, errors=11, skipped=1` — hammasi oldindan mavjud, qarang: `docs/development/LOCAL-VERIFICATION-UZ.md`). To‘liq HTTP/UI sinovlari bloklangan, mahalliy muhitda deps yo‘q.
