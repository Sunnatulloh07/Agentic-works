...
def strict_json(raw):
    ...
    if len(raw) > 1_000_000: raise OAuthError('Provider response exceeds limit')
...
def google_transport(method, url, fields=None, bearer=None):
    ...
    if method == 'POST':
        ...
        if len(data) > 64000: raise OAuthError('Provider request exceeds limit')
    ...
    try:
        ...
        try:
            body = strict_json(error.read(16001))
        except Exception:
            body = {}
        finally:
            error.close()
...
class GoogleOAuth:
    ...
    def __init__(self, config, secret_resolver, transport=google_transport):
        ...
        self.client_id = bounded(config['client_id'], 512)
        ...
        self.redirect = bounded(config['redirect_uri'], 2000)
        ...
        self.expected_account = bounded(config['expected_subject'], 256)
    ...
    def public_config(self):
        return {**self.config, 'scopes': sorted(self.scopes), 'schema_version': 1}
    ...
    def identity(self, token):
        ...
        return bounded(response.get('sub'), 256)
